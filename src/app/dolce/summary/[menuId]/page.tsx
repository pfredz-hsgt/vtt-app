...
import { formatOrdersForWhatsApp, copyToClipboard } from '@/lib/dolce/utils'
import OrderCard from '@/components/dolce/OrderCard' // We will update this component next
import {
    Button,
    Card,
    Grid,
...
export default function SummaryPage() {
    ...
    const stats = useMemo(() => {
        ...
        // Sum total_amount. If total_amount is null (old data), treat as 0
        const totalRevenue = orders.reduce((sum, order) => {
            return sum + (order.total_amount || 0);
        }, 0);

        return { totalCount, paidCount, totalRevenue };
    }, [orders]);
    ...
    return (
        <div style={{ padding: '16px', maxWidth: '800px', margin: '0 auto', paddingBottom: '40px' }}>
            ...
            {/* UPDATED STATS GRID */}
            <Grid columns={3} gap={8} style={{ marginBottom: '24px' }}>
                <Grid.Item>
                    <Card style={{ textAlign: 'center', padding: '12px 4px' }}>
                        <div style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--adm-color-primary)' }}>{stats.totalCount}</div>
                        <div style={{ color: 'var(--adm-color-text-secondary)', fontSize: '12px' }}>Orders</div>
                    </Card>
                </Grid.Item>
                <Grid.Item>
                    <Card style={{ textAlign: 'center', padding: '12px 4px' }}>
                        <div style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--adm-color-success)' }}>{stats.paidCount}</div>
                        <div style={{ color: 'var(--adm-color-text-secondary)', fontSize: '12px' }}>Paid</div>
                    </Card>
                </Grid.Item>
                <Grid.Item>
                    <Card style={{ textAlign: 'center', padding: '12px 4px' }}>
                        <div style={{ fontSize: '20px', fontWeight: 'bold', color: '#f59e0b' }}>
                            <span style={{ fontSize: '14px' }}>RM</span>{stats.totalRevenue.toFixed(0)}
                        </div>
                        <div style={{ color: 'var(--adm-color-text-secondary)', fontSize: '12px' }}>Total</div>
                    </Card>
                </Grid.Item>
            </Grid>
            ...
            <Space direction='vertical' block style={{ marginBottom: '24px' }}>
                <Button
                    ...
                    color='primary'
                    fill='outline'
                    onClick={() => router.push(`/dolce/order/${menuId}`)}
                >
                    <Space>
                        <Plus size={20} />
                        <span>Add Order</span>
            ...
            {orders.length > 0 ? (
                ...
                <div style={{ textAlign: 'center', padding: '40px 0' }}>
                    ...
                    <Button
                        color='primary'
                        onClick={() => {
                            const orderLink = `${window.location.origin}/dolce/latest-order`
